...
export default function DepotMiFinityPage() {
  const t = useTranslations('DepotPages.mifinity');
  ...
  const miFinityCasinos = useMemo(() => {
    return casinos.filter((casino) => !casino.hidden && casino.methods.includes("myfinity")).sort((a, b) => a.rank - b.rank);
  }, []);
  ...
  }, [miFinityCasinos, searchTerm, bonusFilter, wagerFilter, userCountry]);
    ...
    "@graph": [
      {
        ...
        "dateModified": "2026-11-28T00:51:35.118Z",
        "author": {
          "@type": "Person",
  ...
  return (
    <>
      <SEOHead
        ...
        canonical={getTranslatedCanonical('/depot/mifinity', locale)}
        ogTitle={t('ogTitle')}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <section className="glass-card rounded-2xl p-8 mb-12 max-w-5xl mx-auto">
            <div className="space-y-4 text-foreground text-center">
              <p className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.mifinity.intro.p1') }} />
              <p className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.mifinity.intro.p2') }} />
              <div className="flex flex-wrap justify-center gap-4 mt-6 text-sm">
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.instantTransactions')}</span>
                </div>
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.virtualCards')}</span>
                </div>
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.cryptoSupport')}</span>
                </div>
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.regulatedFCA')}</span>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-16 max-w-4xl mx-auto space-y-8">
            <div className="glass-card rounded-2xl p-8">
              ...
              <p className="text-muted-foreground leading-relaxed mb-4" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.mifinity.sections.whatIs.p1') }} />
              <p className="text-muted-foreground leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.mifinity.sections.whatIs.p2') }} />
            </div>
            ...
            <div className="glass-card rounded-2xl p-8">
              <h2 className="text-3xl font-bold mb-4">{t('sections.advantages.title')}</h2>
              <div className="space-y-3 text-muted-foreground">
                <p className="flex items-start gap-2">
                  <span className="text-primary font-bold">✓</span>
                  <span dangerouslySetInnerHTML={{ __html: t('sections.advantages.lowFees') }} />
                </p>
                <p className="flex items-start gap-2">
                  <span className="text-primary font-bold">✓</span>
                  <span dangerouslySetInnerHTML={{ __html: t('sections.advantages.multiCurrency') }} />
                </p>
                <p className="flex items-start gap-2">
                  <span className="text-primary font-bold">✓</span>
                  <span dangerouslySetInnerHTML={{ __html: t('sections.advantages.virtualCard') }} />
                </p>
                <p className="flex items-start gap-2">
                  <span className="text-primary font-bold">✓</span>
                  <span dangerouslySetInnerHTML={{ __html: t('sections.advantages.cryptoSupport') }} />
                </p>
              </div>
            </div>
      ...
      <SEOEnrichedPayment
        ...
        translationNamespace="DepotPages.mifinity.seoEnrichedContent"
      />
      <Footer />
    </>
  );
