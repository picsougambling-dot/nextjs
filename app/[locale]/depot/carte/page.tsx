...
export default function DepotCartePage() {
  const t = useTranslations('DepotPages.carte');
  ...
  const carteCasinos = useMemo(() => {
    return casinos.filter((casino) => !casino.hidden && casino.methods.includes("CB")).sort((a, b) => a.rank - b.rank);
  }, []);
  ...
  }, [carteCasinos, searchTerm, bonusFilter, wagerFilter, userCountry]);
  ...
  "@graph": [
    {
      ...
      "dateModified": "2026-11-28T00:51:35.108Z",
      "author": {
        "@type": "Person",
  ...
  return (
    <>
      <SEOHead
        ...
        canonical={getTranslatedCanonical('/depot/carte', locale)}
        ogTitle={t('ogTitle')}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <section className="glass-card rounded-2xl p-8 mb-12 max-w-5xl mx-auto">
            <div className="space-y-4 text-foreground text-center">
              <p className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.intro.p1') }} />
              <p className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.intro.p2') }} />
              <div className="flex flex-wrap justify-center gap-4 mt-6 text-sm">
                ...
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.secure')}</span>
                </div>
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.universal')}</span>
                </div>
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  <span className="text-primary font-bold">✓</span>
                  <span>{t('advantages.noAccount')}</span>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="mb-6 text-center">
            <p className="text-muted-foreground">
              <span className="text-primary font-semibold">{filteredCasinos.length}</span> {filteredCasinos.length === 1 ? t('results.found', { count: filteredCasinos.length.toString() }) : t('results.foundPlural', { count: filteredCasinos.length.toString() })}
            </p>
          </div>
          ...
          <section className="mt-16 max-w-4xl mx-auto space-y-8">
            <div className="glass-card rounded-2xl p-8">
              <h2 className="text-3xl font-bold mb-4">{t('sections.universal.title')}</h2>
              <p className="text-muted-foreground leading-relaxed mb-4" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.sections.universal.p1') }} />
              <p className="text-muted-foreground leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.sections.universal.p2') }} />
            </div>
            ...
            <div className="glass-card rounded-2xl p-8">
              ...
              <p className="text-muted-foreground leading-relaxed mb-4" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.sections.security.p1') }} />
              <p className="text-muted-foreground leading-relaxed" dangerouslySetInnerHTML={{ __html: getRawTranslation('DepotPages.carte.sections.security.p2') }} />
            </div>
      ...
      <SEOEnrichedPayment
        ...
        translationNamespace="DepotPages.carte.seoEnrichedContent"
      />
      <Footer />
    </>
  );
