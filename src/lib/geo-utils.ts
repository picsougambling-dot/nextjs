...
export async function detectCountryFromIP(request: NextRequest): Promise<string | null> {
  try {
    ...
    // Sur Vercel, l'IP peut être dans plusieurs headers
    const forwardedFor = request.headers.get('x-forwarded-for');
    const vercelForwardedFor = request.headers.get('x-vercel-forwarded-for');
    const realIP = request.headers.get('x-real-ip');
    const cfConnectingIP = request.headers.get('cf-connecting-ip'); // Cloudflare
    
    // Priorité : x-vercel-forwarded-for > x-forwarded-for > x-real-ip > cf-connecting-ip
    let clientIP = vercelForwardedFor?.split(',')[0]?.trim() 
    ...
    if (!clientIP) {
      console.log('[Geo Utils] No IP found in headers:', {
