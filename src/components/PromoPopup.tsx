...
export const PromoPopup = () => {
  ...
  const copyPromoCode = (code: string, casinoName: string) => {
    ...
    toast({
      ...
      description: t('toast.description', { code, casinoName }),
    });
    setTimeout(() => {
  ...
  useEffect(() => {
    // Show popup on initial load after 2 seconds (but not while disclaimer is open)
    ...
    // Show popup every 5 minutes with random casino (but not while disclaimer is open)
  ...
  if (!activePromo || isDisclaimerOpen) return null;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-xl p-0 overflow-hidden border-primary/20">
        <DialogTitle className="sr-only">{activePromo.title}</DialogTitle>
        <Button
          variant="ghost"
          size="icon"
          ...
          <div className="p-4 bg-gradient-to-b from-background/95 to-background">
            ...
            <div className="space-y-3 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                <span className="text-sm text-muted-foreground">{t('availableOn')}</span>
                <img 
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <div className="text-left flex-1 min-w-0">
                            <p className="font-semibold text-sm text-foreground">
                              {codeData.description} {t('onSlot', { slot: codeData.slot })}
                            </p>
                          </div>
                        </div>
              ...
              >
                {t('ctaButton', { casinoName: activePromo.casinoName })}
              </Button>
