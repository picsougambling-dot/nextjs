...
import { ExternalLink, Info, Trophy, Gamepad2, Clock3, Star } from "lucide-react";
...
// Fournisseurs premium qui doivent avoir une étoile
const PREMIUM_PROVIDERS = ["hacksaw", "pragmatic", "nolimit"];
...
export const CasinoCard = ({ casino, index = 0 }: CasinoCardProps) => {
  ...
  const formatBonusText = (bonusText: string): string => {
    ...
    // Formats supportés:
    // - "100% jusqu'à 1 000€ + 100 FS" ou "200% jusqu'à 500€"
    // - "300% jusqu'à 1 500$ + 100 FS"
    // - "225% jusqu'à €1500 & 500 FS"
    // - "150% jusqu'à 1 000$ + 100 FS"
    
    // Pattern 1: Format standard français avec € après le montant
    let match = bonusText.match(/(\d+)%\s+jusqu'à\s+([\d\s]+)€(?:\s+[+&]\s+(\d+)\s+FS)?/);
    if (match) {
      ...
      if (freeSpins) {
        return t('bonusText.withFreeSpins', { percent, amount, freeSpins }).replace(/€/g, currency);
      } else {
        return t('bonusText.withoutFreeSpins', { percent, amount }).replace(/€/g, currency);
      }
    }
    
    // Pattern 2: Format avec $ après le montant
    match = bonusText.match(/(\d+)%\s+jusqu'à\s+([\d\s]+)\$(?:\s+[+&]\s+(\d+)\s+FS)?/);
    if (match) {
      const percent = match[1];
      const amount = match[2].replace(/\s/g, '');
      const freeSpins = match[3];
    ...
    // Pattern 3: Format avec € avant le montant (ex: "€1500")
    match = bonusText.match(/(\d+)%\s+jusqu'à\s+€([\d\s]+)(?:\s+[+&]\s+(\d+)\s+FS)?/);
    if (match) {
      const percent = match[1];
      const amount = match[2].replace(/\s/g, '');
    ...
    // Pattern 4: Format avec ₽ après le montant (ex: "175% jusqu'à 1 000 ₽")
    match = bonusText.match(/(\d+)%\s+jusqu'à\s+([\d\s]+)\s*₽(?:\s+[+&]\s+(\d+)\s+FS)?/);
    if (match) {
      const percent = match[1];
      const amount = match[2].replace(/\s/g, '');
      const freeSpins = match[3];
  ...
  return (
    <TooltipProvider delayDuration={0} skipDelayDuration={0}>
      ...
      >
        ...
        {/* Badge stack: withdrawal -> premium providers -> bookmaker -> bonus */}
        <div className="absolute top-4 right-4 flex flex-nowrap gap-2 justify-end items-start">
          {/* Withdrawal speed - Only show badge for instant withdrawals */}
          {casino.withdrawalSpeed === 'instant' && (
            <Tooltip {...(isTouchDevice ? { open: openTooltip === 'withdrawal', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
              <TooltipTrigger asChild>
                <span className="inline-block" onClick={(e) => { e.stopPropagation(); handleTooltipClick('withdrawal'); }}>
                  <Badge className="bg-emerald-700 text-white font-semibold text-[11px] px-3 py-1 flex items-center gap-1 cursor-help border border-white/10 active:scale-95 transition-transform">
                    <Clock3 className="w-3.5 h-3.5" />
                    {t(`withdrawal.badge.${casino.withdrawalSpeed}`)}
                  </Badge>
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom" sideOffset={8} className="max-w-[260px] z-[100]">
                <p className="text-sm font-semibold mb-1">{t('withdrawal.title')}</p>
                <p className="text-xs text-muted-foreground">{t(`withdrawal.tooltip.${casino.withdrawalSpeed}`)}</p>
              </TooltipContent>
            </Tooltip>
          )}

          {/* Premium providers badge */}
          {casino.providers?.some(p => PREMIUM_PROVIDERS.includes(p)) && (
            <Tooltip {...(isTouchDevice ? { open: openTooltip === 'providers-premium', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
              <TooltipTrigger asChild>
                <span
                  className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm border border-white/15 text-yellow-300 cursor-help active:scale-95 transition-transform shrink-0"
                  onClick={(e) => { e.stopPropagation(); handleTooltipClick('providers-premium'); }}
                >
                  <Star className="w-4 h-4" />
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom" sideOffset={8} className="max-w-[280px] z-[100]">
                <p className="text-xs text-muted-foreground">{t('tooltips.providers.premium')}</p>
              </TooltipContent>
            </Tooltip>
          )}
            ...
            <Tooltip {...(isTouchDevice ? { open: openTooltip === 'bookmaker', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
              <TooltipTrigger asChild>
                <span className="inline-block" onClick={(e) => { e.stopPropagation(); handleTooltipClick('bookmaker'); }}>
                  <Badge className="bg-primary/90 backdrop-blur-sm text-white font-semibold text-xs px-3 py-1 flex items-center gap-1 border border-white/10 cursor-help active:scale-95 transition-transform">
          ...
          <Tooltip {...(isTouchDevice ? { open: openTooltip === 'bonus', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
            <TooltipTrigger asChild>
              <span className="inline-block" onClick={(e) => { e.stopPropagation(); handleTooltipClick('bonus'); }}>
                <Badge className="bg-primary text-primary-foreground font-bold text-base px-3.5 py-1.5 shadow-lg dark:shadow-[0_0_25px_hsl(var(--glow-primary)/0.8)] relative group-hover:animate-pulse cursor-help transition-all duration-300 hover:scale-105 active:scale-95">
                  +{casino.bonusPercent}%
                </Badge>
              </span>
            </TooltipTrigger>
        ...
        <div className="flex-1 flex flex-wrap gap-2 content-start mb-4">
          ...
          {casino.providers && casino.providers.length > 0 && (
            <Tooltip {...(isTouchDevice ? { open: openTooltip === 'providers', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
              ...
              <TooltipContent side="top" sideOffset={8} className="max-w-md z-[100] max-h-[400px] overflow-y-auto overscroll-contain touch-pan-y bg-popover/95 backdrop-blur-lg border border-border shadow-2xl touch-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
                <div className="space-y-3">
                  <div className="flex items-center gap-2 font-semibold text-base mb-3 sticky top-0 bg-popover pb-2 border-b border-border/50 z-10">
