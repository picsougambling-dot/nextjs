...
const methodDetails: Record<string, { min: string; max: string; delay: string; fees: string }> = {
  CB: {
    min: "10€",
    max: "5 000€",
    delay: "Instantané",
    ...
    <TooltipProvider delayDuration={0} skipDelayDuration={0}>
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          {casino.isBookmaker && (
            <Tooltip {...(isTouchDevice ? { open: openTooltip === 'bookmaker', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
              ...
              <TooltipContent side="bottom" sideOffset={8} className="max-w-[280px] z-[100]">
                <p className="text-sm font-semibold mb-1">Paris Sportifs Disponibles</p>
                <p className="text-xs text-muted-foreground">
                  Ce casino propose des paris sportifs : football, tennis, basketball, eSports et plus. 
                  Profitez de cotes compétitives et de paris en direct.
                </p>
              </TooltipContent>
            </Tooltip>
          )}
          <Tooltip {...(isTouchDevice ? { open: openTooltip === 'bonus', onOpenChange: (open) => !open && setOpenTooltip(null) } : {})}>
            ...
            <TooltipContent side="bottom" sideOffset={8} className="max-w-[260px] z-[100]">
              <p className="text-sm font-semibold mb-1">Calcul du Bonus</p>
              <p className="text-xs text-muted-foreground">
                Bonus de {casino.bonusPercent}% sur votre dépôt. Exemple : déposez 100€, recevez {casino.bonusPercent}€ de bonus !
              </p>
            </TooltipContent>
          </Tooltip>
        </div>
      ...
      <div className="p-6 flex flex-col h-[280px]">
        <div className="flex-none mb-4">
          ...
          <p className="text-muted-foreground text-sm">{casino.bonusText}</p>
        </div>
            ...
            <TooltipContent side="top" sideOffset={8} className="max-w-sm z-[100]">
              <div className="space-y-2">
                ...
                <div className="text-xs space-y-1.5 text-muted-foreground">
                  {casino.wager === null ? (
                    <>
                      <p className="text-green-500 dark:text-green-400 font-medium">✓ Aucune condition de mise !</p>
                      <p>Retirez vos gains immédiatement sans contrainte.</p>
                    </>
                  ) : (
                    <>
                      <p>Vous devez miser <span className="font-semibold text-foreground">{casino.wager}x</span> le montant du bonus.</p>
                      <div className="mt-2 pt-2 border-t border-border/50">
                        <p className="text-[10px] opacity-70">
                          Exemple: Bonus de 100€ = {casino.wager * 100}€ à miser
                        </p>
                      </div>
                    </>
                  )}
              ...
              <TooltipTrigger asChild>
                <span className="inline-block" onClick={(e) => { e.stopPropagation(); handleTooltipClick('providers'); }}>
                  <Badge className="bg-gradient-to-r from-secondary/80 to-secondary/60 text-secondary-foreground font-semibold cursor-help transition-all hover:scale-105 hover:shadow-lg flex items-center gap-1.5 border border-secondary/30 active:scale-95">
                    ...
                    {casino.providers.length} Fournisseurs
                  </Badge>
                </span>
              </TooltipTrigger>
              <TooltipContent side="top" sideOffset={8} className="max-w-md z-[100] max-h-[400px] overflow-y-auto scrollbar-thin bg-popover/95 backdrop-blur-lg border border-border shadow-2xl">
                <div className="space-y-3">
                  <div className="flex items-center gap-2 font-semibold text-base mb-3 sticky top-0 bg-popover pb-2 border-b border-border/50">
                    ...
                    <span>Fournisseurs de Jeux ({casino.providers.length})</span>
                  </div>
                  ...
                  {casino.providers.some(p => ["pragmatic", "evolution", "netent", "hacksaw", "nolimit", "playngo"].includes(p)) && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-3 pt-3 border-t border-border/50">
                      ...
                      <span>Inclut des fournisseurs premium reconnus</span>
                    </div>
                  )}
                </div>
              </TooltipContent>
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase tracking-wider opacity-60">Limites</p>
                        <p className="font-medium text-foreground">
                          {details.min} - {details.max}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase tracking-wider opacity-60">Délai</p>
                        <p className="font-medium text-foreground">
                          {details.delay}
                        </p>
                      </div>
                      <div className="col-span-2 pt-1.5 border-t border-border/50 space-y-1">
                        <p className="text-[10px] uppercase tracking-wider opacity-60">Frais</p>
