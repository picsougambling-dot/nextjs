...
import { Search, Dices, RotateCcw, Check, ChevronsUpDown, Percent, TrendingDown, Clock, CreditCard, Trophy, Gamepad2 } from "lucide-react";
...
export const FilterBar = ({
  ...
  const hasActiveFilters = searchTerm !== "" || bonusFilter !== "all" || wagerFilter !== "all" || 
    ...
    (providerFilter && providerFilter !== "all") || (withdrawalFilter && withdrawalFilter !== "all");
  ...
  const handleReset = () => {
    ...
    if (onWithdrawalChange) onWithdrawalChange("all");
  };
  ...
  const showWithdrawalFilter = withdrawalFilter !== undefined && onWithdrawalChange !== undefined;
  
  const filterCount = 3
    + (showMethodFilter ? 1 : 0)
    + (showBookmakerFilter ? 1 : 0)
    + (showProviderFilter ? 1 : 0)
    + (showWithdrawalFilter ? 1 : 0);

  // Force all filtres sur une ligne en desktop (jusqu'à 7 éléments)
  const gridCols =
    filterCount >= 7 ? 'lg:grid-cols-7'
    : filterCount === 6 ? 'lg:grid-cols-6'
    : filterCount === 5 ? 'lg:grid-cols-5'
    : filterCount === 4 ? 'lg:grid-cols-4'
    : 'lg:grid-cols-3';
    ...
    <div className="glass-card rounded-2xl p-4 sm:p-6 mb-8 space-y-4">
      ...
      {filteredCount > 0 && (
        <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground pb-2 border-b border-border/50">
          ...
          <span>
            <CounterAnimation end={filteredCount} duration={800} /> {t('casinosFound', { count: filteredCount })}
          </span>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 ${gridCols}`}>
        ...
        <Select value={bonusFilter} onValueChange={onBonusChange}>
          <SelectTrigger className="bg-background/50 border-border focus:border-primary h-10" aria-label={t('bonusFilter')}>
            <div className="flex items-center gap-2">
              <Percent className="w-4 h-4 text-muted-foreground shrink-0" />
              <SelectValue placeholder={t('bonusFilter')} />
            </div>
          </SelectTrigger>
          <SelectContent className="max-h-[250px] sm:max-h-[300px]">
            <SelectItem value="all">{t('allBonuses')}</SelectItem>
        ...
        <Select value={wagerFilter} onValueChange={onWagerChange}>
          <SelectTrigger className="bg-background/50 border-border focus:border-primary h-10" aria-label={t('wagerFilter')}>
            <div className="flex items-center gap-2">
              <TrendingDown className="w-4 h-4 text-muted-foreground shrink-0" />
              <SelectValue placeholder={t('wagerFilter')} />
            </div>
          </SelectTrigger>
          <SelectContent className="max-h-[250px] sm:max-h-[300px]">
            <SelectItem value="all">{t('allWagers')}</SelectItem>
            <SelectItem value="none">{t('noWager')}</SelectItem>
        ...
        {showWithdrawalFilter && withdrawalFilter && onWithdrawalChange && (
          <Select value={withdrawalFilter} onValueChange={onWithdrawalChange}>
            <SelectTrigger className="bg-background/50 border-border focus:border-primary h-10" aria-label={t('withdrawalFilter')}>
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-muted-foreground shrink-0" />
                <SelectValue placeholder={t('withdrawalFilter')} />
              </div>
            </SelectTrigger>
            <SelectContent className="max-h-[250px] sm:max-h-[300px]">
              <SelectItem value="all">{t('allWithdrawals')}</SelectItem>
              <SelectItem value="instant">{t('withdrawal.instant')}</SelectItem>
              <SelectItem value="48h">{t('withdrawal.48h')}</SelectItem>
            </SelectContent>
          </Select>
        )}
        ...
        {showMethodFilter && methodFilter && onMethodChange && (
          <Select value={methodFilter} onValueChange={onMethodChange}>
            <SelectTrigger className="bg-background/50 border-border focus:border-primary h-10" aria-label={t('paymentMethod')}>
              <div className="flex items-center gap-2">
        ...
        {showBookmakerFilter && bookmakerFilter && onBookmakerChange && (
          <Select value={bookmakerFilter} onValueChange={onBookmakerChange}>
            <SelectTrigger className="bg-background/50 border-border focus:border-primary h-10" aria-label={t('casinoType')}>
              <div className="flex items-center gap-2">
                <Trophy className="w-4 h-4 text-muted-foreground shrink-0" />
                <SelectValue placeholder={t('casinoType')} />
              </div>
            </SelectTrigger>
            <SelectContent className="max-h-[250px] sm:max-h-[300px]">
              <SelectItem value="all">{t('allCasinos')}</SelectItem>
              <SelectItem value="bookmaker">{t('casinoBookmaker')}</SelectItem>
              <SelectItem value="casino-only">{t('casinoOnly')}</SelectItem>
            </SelectContent>
          </Select>
        )}
                ...
                <CommandList className="max-h-[250px] sm:max-h-[350px] overflow-y-auto scrollbar-thin scrollbar-thumb-primary/30 scrollbar-track-muted/20">
                  ...
                  <CommandGroup className="p-2">
                    ...
                    >
                      ...
                      />
                      <span className="font-medium text-sm">{t('allProviders')}</span>
                    </CommandItem>
      ...
      {hasActiveFilters && (
        <div className="flex justify-center pt-2">
          ...
          >
            ...
            <span className="inline sm:hidden">{t('reset')}</span>
          </Button>
        </div>
      )}
    </div>
