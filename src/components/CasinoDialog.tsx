...
import { ExternalLink, CreditCard, DollarSign, Percent, TrendingDown, Clock3 } from "lucide-react";
...
export const CasinoDialog = ({ casino, open, onOpenChange }: CasinoDialogProps) => {
  const t = useTranslations('CasinoDialog');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-card border-primary/20 overflow-y-auto fixed inset-0 sm:inset-auto left-0 top-0 translate-x-0 translate-y-0 m-0 w-screen h-[100dvh] max-w-none rounded-none sm:left-1/2 sm:top-1/2 sm:translate-x-[-50%] sm:translate-y-[-50%] sm:w-full sm:max-w-2xl sm:h-auto sm:max-h-[90vh] sm:rounded-lg">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-4 pt-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <DialogDescription className="text-sm sm:text-base">
                {t('description', { percent: casino.bonusPercent })}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
          ...
          <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-sm sm:text-base text-foreground">{t('bonusTitle')}</h3>
            </div>
            <div className="space-y-2">
              <Badge className="bg-primary text-primary-foreground font-bold text-base sm:text-lg px-3 sm:px-4 py-1.5 sm:py-2">
                +{casino.bonusPercent}%
              </Badge>
              <p className="text-xs sm:text-sm text-muted-foreground mt-2">{formatBonusText(casino.bonusText)}</p>
            </div>
          </div>
          ...
          <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-sm sm:text-base text-foreground">{t('wagerTitle')}</h3>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mt-2">
                {casino.wager === null 
                  ? t('wagerNoCondition') 
                  : t('wagerCondition', { wager: casino.wager })}
              </p>
            </div>
          </div>

          {/* Withdrawal speed - Only show for instant withdrawals */}
          {casino.withdrawalSpeed === 'instant' && (
            <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20">
              <div className="flex items-center gap-2 mb-3">
                <Clock3 className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
                <h3 className="font-semibold text-sm sm:text-base text-foreground">{t('withdrawalTitle')}</h3>
              </div>
              <div className="space-y-2">
                <Badge className="bg-emerald-700 text-white font-semibold text-base sm:text-lg px-3 sm:px-4 py-1.5 sm:py-2 flex items-center gap-2">
                  <Clock3 className="w-4 h-4" />
                  {t(`withdrawal.badge.${casino.withdrawalSpeed}`)}
                </Badge>
                <p className="text-xs sm:text-sm text-muted-foreground">
                  {t(`withdrawal.description.${casino.withdrawalSpeed}`)}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-sm sm:text-base text-foreground">{t('minDepositTitle')}</h3>
            </div>
            <div className="space-y-2">
              <p className="text-xl sm:text-2xl font-bold text-primary">{casino.minDeposit}</p>
              <p className="text-xs sm:text-sm text-muted-foreground">
                {t('minDepositDescription')}
              </p>
            </div>
          </div>
          ...
          <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-sm sm:text-base text-foreground">{t('paymentMethodsTitle')}</h3>
            </div>
        ...
        <div className="glass-card p-3 sm:p-4 rounded-xl border border-primary/20 mb-6">
          <h3 className="font-semibold text-sm sm:text-base text-foreground mb-2">{t('importantInfoTitle')}</h3>
          <ul className="space-y-2 text-xs sm:text-sm text-muted-foreground">
            <li>• {t('importantInfo.newPlayers')}</li>
            <li>• {t('importantInfo.termsApply')}</li>
            <li>• {t('importantInfo.responsibleGaming')}</li>
            <li>• {t('importantInfo.freeSpinsConditions')}</li>
          </ul>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <a
              ...
              aria-label={t('ctaPlayAriaLabel', { name: casino.name })}
            >
          ...
          >
            {(() => {
              ...
              const casinosWithPages = [
                ...
                'madcasino', 'kingdom', 'hispin', 'machance', 'slotsandcasino', '31bets', 'orionsbet', 'jumbabet', 'bigdollar', 'slotsofvegas', '7kcasino', 'spinnycasino', 'lunarspins', 'freespinza'
              ];
              let casinoSlug = casino.name.toLowerCase().replace(/\s+/g, '').replace(/\./g, '');
              ...
              return <a href={href}>{t('ctaOtherOffers')}</a>;
            })()}
          </Button>
        </div>
      </DialogContent>
